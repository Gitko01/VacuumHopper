...
public class VacuumFilterScreen extends HandledScreen<VacuumFilterScreenHandler> {
    private static final Identifier TEXTURE = new Identifier(VacuumHopper.MOD_ID, "textures/gui/container/vacuum_filter_gui.png");
    ...
    protected void init() {
        ...
        if (modeButton == null && mode != -1) {
            ...
            modeButton = this.addDrawableChild(createModeWidget(
                    ...
                    (button, matrices, mouseX, mouseY) -> renderTooltip(matrices, Text.translatable("gui." + VacuumHopper.MOD_ID + ".vacuum_filter.mode." + mode), mouseX, mouseY),
                    Text.literal(""), client));
        }

        if (mode == -1) {
            VacuumHopper.LOGGER.error("[Vacuum Hopper] Mode for a vacuum filter is -1! (Failed to get value from screen handler)");
        }
    }

    public CustomTexturedButtonWidget createModeWidget(int x, int y, int xMargin, int yMargin, int width, int height, int u, int v, int hoveredVOffset, Identifier guiTexture, int guiTextureWidth, int guiTextureHeight, ButtonWidget.PressAction pressAction, ButtonWidget.TooltipSupplier tooltipSupplier, Text text, MinecraftClient client) {
        return new CustomTexturedButtonWidget(x + xMargin, y + yMargin, width, height, u, v, hoveredVOffset, guiTexture, guiTextureWidth, guiTextureHeight, pressAction, tooltipSupplier, text) {
            @Override
            public void onPress() {
                if (mode == 1) {
                    mode = 0;
    ...
    private void updateItemsToFilter(Hashtable<Integer, ItemStack> itemsToFilter, MinecraftClient client) {
        client.execute(() -> {
            ...
            ClientPlayNetworking.send(new Identifier(VacuumHopper.MOD_ID, "update_vacuum_filter_items_packet"), buf);
        });
    }
    ...
    private void updateMode(int newMode, MinecraftClient client) {
        client.execute(() -> {
            ...
            ClientPlayNetworking.send(new Identifier(VacuumHopper.MOD_ID, "update_vacuum_filter_mode_packet"), buf);
        });
    }
