package net.gitko.vacuumhopper.block.custom;
...
public class VacuumHopperBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlocks.VACUUM_HOPPER_BLOCK_ENTITY, (world1, pos, state1, be) -> VacuumHopperBlockEntity.tick(world1, pos, state1, be));
    }

    @Override
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.getInventory().getStack(player.getInventory().selectedSlot).getItem() != ModItems.VACUUM_SCREWDRIVER) {
            if (!world.isClient) {
