...
public class VacuumHopperScreen extends HandledScreen<VacuumHopperScreenHandler> {
    private static final Identifier TEXTURE = new Identifier(VacuumHopper.MOD_ID, "textures/gui/container/vacuum_hopper_gui.png");
    ...
    protected void init() {
        ...
        if (redstoneModeButton == null && redstoneMode != -1) {
            ...
            redstoneModeButton = this.addDrawableChild(createRedstoneModeWidget(
                    ...
                    (button, matrices, mouseX, mouseY) -> renderTooltip(matrices, Text.translatable("gui." + VacuumHopper.MOD_ID + ".vacuum_hopper.redstoneMode." + redstoneMode), mouseX, mouseY),
                    Text.literal(""), client));
        }
        ...
        if (pushModeButton == null && pushMode != -1) {
            ...
            pushModeButton = this.addDrawableChild(createPushModeWidget(
                    ...
                    (button, matrices, mouseX, mouseY) -> renderTooltip(matrices, Text.translatable("gui." + VacuumHopper.MOD_ID + ".vacuum_hopper.pushMode." + pushMode), mouseX, mouseY),
                    Text.literal(""), client));
        }
        ...
        if (rangeScroller == null) {
            rangeScroller = this.addDrawableChild(createVacuumRangeWidget(
                    x, y, 61, 58, 90, 20, Text.translatable("gui." + VacuumHopper.MOD_ID + ".vacuum_hopper.rangeSlider"), range, VacuumHopperBlockEntity.MAX_REACH, client)
            );
        }

        if (redstoneMode == -1) {
            VacuumHopper.LOGGER.error("[Vacuum Hopper] Redstone mode for a vacuum hopper is -1! (Failed to get value from screen handler)");
        }

        if (pushMode == -1) {
            VacuumHopper.LOGGER.error("[Vacuum Hopper] Push mode for a vacuum hopper is -1! (Failed to get value from screen handler)");
        }

        if (range == -1) {
            VacuumHopper.LOGGER.error("[Vacuum Hopper] Range for a vacuum hopper is -1! (Failed to get value from screen handler)");
        }
    }

    public CustomTexturedButtonWidget createRedstoneModeWidget(int x, int y, int xMargin, int yMargin, int width, int height, int u, int v, int hoveredVOffset, Identifier guiTexture, int guiTextureWidth, int guiTextureHeight, ButtonWidget.PressAction pressAction, ButtonWidget.TooltipSupplier tooltipSupplier, Text text, MinecraftClient client) {
        CustomTexturedButtonWidget buttonWidget = new CustomTexturedButtonWidget(x + xMargin, y + yMargin, width, height, u, v, hoveredVOffset, guiTexture, guiTextureWidth, guiTextureHeight, pressAction, tooltipSupplier, text) {
            @Override
            public void onPress() {
                if (redstoneMode == 2) {
                    redstoneMode = 0;
    ...
    public CustomTexturedButtonWidget createPushModeWidget(int x, int y, int xMargin, int yMargin, int width, int height, int u, int v, int hoveredVOffset, Identifier guiTexture, int guiTextureWidth, int guiTextureHeight, ButtonWidget.PressAction pressAction, ButtonWidget.TooltipSupplier tooltipSupplier, Text text, MinecraftClient client) {
        CustomTexturedButtonWidget buttonWidget = new CustomTexturedButtonWidget(x + xMargin, y + yMargin, width, height, u, v, hoveredVOffset, guiTexture, guiTextureWidth, guiTextureHeight, pressAction, tooltipSupplier, text) {
            @Override
            public void onPress() {
                if (pushMode == 1) {
    ...
    private void updateRange(int newRange, MinecraftClient client) {
        client.execute(() -> {
            ...
            ClientPlayNetworking.send(new Identifier(VacuumHopper.MOD_ID, "update_vacuum_hopper_reach_packet"), buf);
        });
    }
    ...
    private void updateRedstoneMode(int newRedstoneMode, MinecraftClient client) {
        client.execute(() -> {
            ...
            ClientPlayNetworking.send(new Identifier(VacuumHopper.MOD_ID, "update_vacuum_hopper_redstone_mode_packet"), buf);
        });
    }
    ...
    private void updatePushMode(int newPushMode, MinecraftClient client) {
        client.execute(() -> {
            ...
            ClientPlayNetworking.send(new Identifier(VacuumHopper.MOD_ID, "update_vacuum_hopper_push_mode_packet"), buf);
        });
    }
